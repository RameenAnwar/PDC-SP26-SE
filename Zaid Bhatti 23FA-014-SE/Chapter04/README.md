...
- **Chapter Focus:** Distributed Memory Parallelism and Message Passing Interface (MPI)

---

## 1. Overview of Distributed Computing

In previous chapters, we analyzed shared memory concurrency and multi-core parallelism confined within a single physical machine. In this chapter, we explore **Distributed Computing**, where calculations run across multiple separate computers connected over a network.

We use the **Message Passing Interface (MPI)** standard with the `mpi4py` library to coordinate executions and share data between isolated nodes.
...
### Deadlocks & Cartesian Topologies
- **Deadlock:** Occurs when processes are blocked indefinitely, each waiting for another to perform an action.
- **Virtual Topologies:** Allows linear ranks to map to 2D or 3D grids to simplify neighbor calculations for spatial computations.

---

## 5. Practical Implementation Analysis

### Basic MPI Initialization (`helloworld_MPI.py`)
```python
...
### Point-to-Point Messaging (`pointToPointCommunication.py`)
```python
from mpi4py import MPI
...
### Broadcasting Values (`broadcast.py`)
```python
from mpi4py import MPI
...
### Gathering Data Pools (`gather.py`)
```python
from mpi4py import MPI
...
### Matrix All-to-All Operations (`alltoall.py`)
```python
from mpi4py import MPI
import numpy as np
...
### Reduction Algorithms (`reduction.py`)
```python
from mpi4py import MPI
import numpy as np
...
### Deadlock Demonstration (`deadLockProblems.py`)
```python
from mpi4py import MPI
...
### Grid Virtual Topologies (`virtualTopology.py`)
```python
from mpi4py import MPI
